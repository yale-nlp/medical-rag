...
async def run_async_evaluation(annotator, all_cases: list, concurrency_limit: int = 100):
    print(f"--- {type(annotator).__name__}: Preparing all prompts for async execution (Concurrency: {concurrency_limit}) ---")
    ...
    print(f"\n--- Sending {len(all_tasks)} requests to API with controlled concurrency ---")
...
async def main():
    ...
    parser.add_argument("--concurrency", type=int, default=100, help="Concurrency limit for async API calls.")
    ...
    # [MODIFIED] Simplified branching logic
    if hasattr(annotator, 'evaluate_async'):
        all_cases = await run_async_evaluation(annotator, all_cases, concurrency_limit=args.concurrency)
    else: # For HuggingFace (or any other sync annotator)
        print(f"--- {type(annotator).__name__}: Running Sync Evaluations ---")
...
# from pathlib import Path
# from tqdm import tqdm
# from gemini_annotator import get_annotator
# import numpy as np
# from collections import Counter
...
# def run_annotation_on_case(annotator, case_data: dict) -> dict:
#     case_id, original_report, original_keywords = case_data.get("case_id"), case_data.get("original_reviewer_report"), case_data.get("original_keywords", [])
#     if not all([original_report, original_keywords]): return {"error": "Missing data"}
...
#         for paper in case_data.get("evidence_reranked_papers", {}).get(kw, []):
#             paper_ctx = {**kw_ctx, "current_paper_title": paper.get("title", "N/A"), "current_paper_abstract": paper.get("abstract", "N/A")}
#             paper_url = paper.get("url") or f"no_url_paper_{paper.get('title', 'N/A')}"
#             results["paper_relevance"][paper_url] = _to_int_score(annotator.evaluate(annotator._construct_prompt("reranked_results_evaluation", paper_ctx)).get("score"))
        
#         summary = case_data.get("generated_textbook_summaries", {}).get(kw)
#         if summary: 
#             results["textbook_summary_quality"][kw] = _to_int_score(annotator.evaluate(annotator._construct_prompt("textbook_summary_evaluation", {**kw_ctx, "textbook_summary_text": summary})).get("score"))
...
#     print(f"--- Starting Evaluation with: {type(annotator).__name__} ---")
#     for case in tqdm(all_cases, desc=f"Annotating with {model_name}"):
#         case['annotation'] = run_annotation_on_case(annotator, case)
