...
#     def evaluate(self, prompt_text: str, max_retries: int = 5, initial_delay: int = 1) -> dict:
#         delay = initial_delay
#         for attempt in range(max_retries):
#             try:
...
#                     print(f"[WARN] OpenAI Rate limit hit. Attempt {attempt + 1}/{max_retries}. Retrying in {wait_time:.2f}s...")
#                     if attempt + 1 == max_retries: return {"error": f"Rate limit exceeded after {max_retries} retries.", "details": str(e)}
...
#         for attempt in range(max_retries):
#             try:
#                 response = self.model.generate_content(prompt_text, generation_config={"temperature": 0.0})
#                 raw_output = response.text.strip()
#                 score_match = re.search(r'\d', raw_output)
#                 if score_match:
#                     return {"score": int(score_match.group(0))}
...
#                 if "429" in str(e) or "Resource has been exhausted" in str(e):
#                     wait_time = delay + random.uniform(0, 1)
#                     print(f"[WARN] Gemini Rate limit hit. Attempt {attempt + 1}/{max_retries}. Retrying in {wait_time:.2f}s...")
#                     if attempt + 1 == max_retries: return {"error": f"Rate limit exceeded after {max_retries} retries.", "details": str(e)}
#                     time.sleep(wait_time)
#                     delay *= 2
#                 else:
